...
import { REST, TaskMonitor } from '@spinnaker/core';

import { ALICLOUD_FOOTER_DIRECTIVE } from '../../../common/footer.directive';
import { SERVER_GROUP_WRITER_ALI } from '../serverGroupWirter';

export const ALICLOUD_SERVERGROUP_DETAIL_UPDATELAUNCHCONFIG =
  'spinnaker.alicloud.serverGroup.details.updateLaunchConfig.controller';
angular
  .module(ALICLOUD_SERVERGROUP_DETAIL_UPDATELAUNCHCONFIG, [
    // SERVER_GROUP_WRITER,
    SERVER_GROUP_WRITER_ALI,
    ALICLOUD_FOOTER_DIRECTIVE,
  ])
  .controller('alicloudUpdateLaunchConfigServerGroupCtrl', [
    ...
    function ($scope: any, $uibModalInstance: any, serverGroupWriters: any, application: any, serverGroup: any) {
      ...
      $scope.$watch(
        'serverGroup.scalingConfigurations.tags',
      ...
      $scope.$watch('serverGroup.result.scalingConfiguration.internetMaxBandwidthOut', function (
        oldVal: any,
        newVal: any,
      ) {
        if (newVal !== oldVal) {
      ...
      $scope.$watch('serverGroup.result.scalingConfiguration.spotPriceLimit[0].priceLimit', function (
        oldVal: any,
        newVal: any,
      ) {
        if (newVal !== oldVal) {
          ...
          REST('/subnets/alicloud')
            ...
            .then((vnets: any) => {
              ...
              types.forEach((item: any) => {
                if (
                  item.account === $scope.serverGroup.account &&
                  item.regionId === $scope.serverGroup.region &&
                  item.zoneId === zoneIds
                ) {
      ...
      this.submit = function () {
        if ($scope.serverGroup.result.scalingConfiguration.spotStrategy !== 'SpotWithPriceLimit') {
          if (
            $scope.serverGroup.result.scalingConfiguration.spotPriceLimit &&
            $scope.serverGroup.result.scalingConfiguration.spotPriceLimit.length > 0
          ) {
