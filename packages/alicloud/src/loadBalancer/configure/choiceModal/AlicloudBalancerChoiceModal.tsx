...
import type { IAlicloudLoadBalancerConfig } from './LoadBalancerTypes';
import { LoadBalancerTypes } from './LoadBalancerTypes';
import { AliCloudProviderSettings } from '../../../alicloud.settings';

export interface IAlicloudLoadBalancerChoiceModalState {
  choices: IAlicloudLoadBalancerConfig[];
  selectedChoice: IAlicloudLoadBalancerConfig;
}

export class AlicloudLoadBalancerChoiceModal extends React.Component<
  ILoadBalancerModalProps,
  IAlicloudLoadBalancerChoiceModalState
> {
  ...
  public static show(props: ILoadBalancerModalProps): Promise<void> {
    return ReactModal.show(
      AlicloudLoadBalancerChoiceModal,
      {
        ...props,
  ...
  public choiceSelected(choice: IAlicloudLoadBalancerConfig): void {
  ...
  private getIncompatibility(choice: IAlicloudLoadBalancerConfig, cloudProvider: string): ILoadBalancerIncompatibility {
  ...
  private isIncompatibleWithAllProviders(choice: IAlicloudLoadBalancerConfig) {
    const {
      app: { attributes },
    } = this.props;
  ...
  public render() {
    ...
    const loadBalancerWarning =
      AliCloudProviderSettings.createLoadBalancerWarnings &&
      AliCloudProviderSettings.createLoadBalancerWarnings[selectedChoice.type];
