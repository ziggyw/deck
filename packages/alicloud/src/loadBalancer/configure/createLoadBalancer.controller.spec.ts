...
import { ApplicationModelBuilder, SETTINGS } from '@spinnaker/core';
...
describe('Controller: alicloudCreateLoadBalancerCtrl', function () {
  ...
  it('makes the expected REST calls for data for a new loadbalancer', function () {
    $http.when('GET', SETTINGS.gateUrl + '/networks').respond([]);
    $http.when('GET', SETTINGS.gateUrl + '/securityGroups').respond({});
    $http.when('GET', SETTINGS.gateUrl + '/credentials?expand=true').respond([]);
    $http.when('GET', SETTINGS.gateUrl + '/subnets').respond([]);
  });
});
