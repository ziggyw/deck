...
export class ALBLocation extends React.Component<IALBLocationProps, IALBLocationStates> {
  ...
  public componentDidMount() {
    ...
    REST('/subnets/alicloud')
      ...
      .then((res: any) => {
        res?.map((element: any) => {
          if (!zoneIds[element.zoneId]) {
            zoneIds[element.zoneId] = new Set();
          }
          if (!vpcIds[element.vpcId]) {
            vpcIds[element.vpcId] = new Set();
          }
          if (!regionIds[element.region]) {
            regionIds[element.region] = new Set();
          }
          zoneIds[element.zoneId].add(element.vswitchId);
          vpcIds[element.vpcId].add(element.region);
          regionIds[element.region].add(element.zoneId);
        });

        this.setState({
          vpcIdsOpt: Object.keys(vpcIds).map((vpc) => ({ label: vpc, value: vpc })),
          vpcIds,
          zoneIds,
        });
      });
  ...
  private handleVpcUpdated = (vpcId: any) => {
    const { vpcIds } = this.state;
    this.setState({
      regionIdsOpt: Array.from(vpcIds[vpcId.value]).map((region) => ({ label: region, value: region })),
    });
  ...
  private handleZoneIdAdd = (ZoneId: any) => {
    ...
    this.setState({
      vSwitchZoneIds: Object.keys(zoneIds).map((zoneId) => {
        zoneIds[zoneId];
        return {
          zoneId,
          vSwitchs: Array.from(zoneIds[zoneId]).map((vSwitch) => ({ label: vSwitch, value: vSwitch })),
        };
      }),
    });
  };
  ...
  public render() {
    ...
    const { accounts, vSwitchZoneIds, regions, regionIdsOpt, vpcIdsOpt }: any = this.state;
            ...
            <div className="form-group">
              ...
              <div className="col-md-7">
                <Select
                  ...
                  value={values.vpcId}
                  options={vpcIdsOpt}
                  onChange={this.handleVpcUpdated}
                />
              </div>
            </div>
