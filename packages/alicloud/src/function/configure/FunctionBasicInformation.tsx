...
import { s3BucketNameValidator } from '../alicoud.validators';
...
export class FunctionBasicInformation
  ...
  public componentDidMount(): void {
    ...
    const allAccounts$ = observableFrom(AccountService.listAccounts('alicloud')).pipe(shareReplay(1));
    ...
    const allFunctions$ = this.props.app.getDataSource('functions').data$ as Observable<AlicloudFunction[]>;
      ...
      <div className="container-fluid form-horizontal ">
        {isNew && (
          <div className={className}>
            ...
            <HelpField id="alicloud.function.name" />
            <span>
        ...
        <FormikFormField
          name="functionName"
          label="Function Name"
          help={<HelpField id="alicloud.function.name" />}
          input={(props) => <TextInput {...props} />}
        />
        <FormikFormField
          name="runtime"
          label="Runtime"
          help={<HelpField id="alicloud.function.runtime" />}
          input={(props) => <ReactSelectInput {...props} stringOptions={availableRuntimes} clearable={true} />}
        />
        <FormikFormField
          name="s3bucket"
          label="S3 Bucket"
          help={<HelpField id="alicloud.function.s3bucket" />}
          input={(props) => <TextInput {...props} placeholder="S3 bucket name" />}
        />
        <FormikFormField
          name="s3key"
          label="S3 Key"
          help={<HelpField id="alicloud.function.s3key" />}
          input={(props) => <TextInput {...props} placeholder="object.zip" />}
        />
        <FormikFormField
          name="handler"
          label="Handler"
          help={<HelpField id="alicloud.function.handler" />}
