...
export class Network
  ...
  public validate(): FormikErrors<AlicloudFunctionUpsertCommand> {
    return {};
  }

  private getAvailableSubnets(): PromiseLike<ISubnet[]> {
    return SubnetReader.listSubnetsByProvider('alicloud');
  }
  ...
  private getSecurityGroupsByVpc = (sgs: ISecurityGroupsByAccountSourceData): Array<Option<string>> => {
    ...
    forOwn(sgs, function (sgByAccount, acc) {
      if (acc === values.credentials) {
        /** Get security groups that fall under the provider 'alicloud' */
        ...
        <div className="col-md-11">
          <div className="sp-margin-m-bottom">
            {values.credentials && (
              <FormikFormField
                name="vpcId"
                label="VPC Id"
                help={<HelpField id="alicloud.function.vpc.id" />}
                input={(props) => (
                  <ReactSelectInput
                    {...props}
          ...
          <div className="form-group">
            <div className="col-md-4 sm-label-right">
              <b>Subnets </b>
              <HelpField id="alicloud.function.subnet" />
            </div>
